# Мультивалютность — Отчёт и инструкции

## Что сделано на бэкенде

### Модели
- **Currency**: добавлены поля `symbol` (₽, $, €), `rate_to_rub`, `updated_at`
- **City**: добавлены поля `country`, `currency` (FK)
- **Vehicle**: добавлено поле `currency` (FK) с авто-определением по городу

### Сервис курсов валют
- Источник: **ExchangeRate-API v6** (с API ключом)
- Обновление: автоматически каждый час через Celery
- 44 валюты поддерживаются

### API endpoints
- `GET /api/currency/` — список валют с символами и курсами
- `POST /api/currency/update-rates/` — ручное обновление (только админ)
- `GET /api/currency/convert/?amount=X&from=USD&to=RUB` — конвертация

### Сериализаторы
- Транспорт возвращает `currency` и `user_currency`
- Цены возвращают `price_converted` и `total_converted`

---

## Что нужно сделать администратору

1. Зайти в админку: `/admin/franchise/city/`
2. Для каждого города указать:
   - **Страна** (текстовое поле)
   - **Валюта** (выбор из списка с автокомплитом)
3. Примеры:
   - Пхукет → Таиланд → THB (฿)
   - Москва → Россия → RUB (₽)
   - Дубай → ОАЭ → AED (د.إ)

После этого новые транспорты в этих городах автоматически получат валюту города.
...
### 1. Курсы валют
- [ ] `GET /api/currency/` — проверить что symbol и rate_to_rub заполнены
- [ ] `GET /api/currency/convert/?amount=100&from=USD&to=RUB` — проверить конвертацию

### 2. Транспорт
- [ ] Создать транспорт в городе с валютой → проверить что currency заполнилась
- [ ] `GET /api/vehicles/auto/{id}/` — проверить что возвращается `currency`
- [ ] `GET /api/vehicles/auto/?currency=USD` — проверить `*_converted` поля

### 3. Админка
- [ ] `/admin/franchise/city/` — проверить что видны поля country, currency
- [ ] `/admin/app/currency/` — проверить что видны symbol, rate_to_rub
